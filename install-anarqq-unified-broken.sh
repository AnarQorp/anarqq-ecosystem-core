...
log_message() {
    ...
    case "$level" in
        "ERROR") 
            print_error "$message"
            [ -n "$context" ] && print_info "Context: $context"
            ;;
        "WARN") 
        ...
        "INFO") 
            ...
            ;;
        "DEBUG") 
            [ "${INSTALL_CONFIG[verbose]}" = "true" ] && print_substep "$message"
            ;;
        "SUCCESS")
...
initialize_system_info() {
    ...
    log_message "DEBUG" "Shell: ${SYSTEM_CAPS[shell]}"
    log_message "DEBUG" "Docker: ${SYSTEM_CAPS[has_docker]}"
    log_message "DEBUG" "Git: ${SYSTEM_CAPS[has_git]}"
...
check_dependencies() {
    ...
    for dep in "${optional_deps[@]}"; do
        if ! command_exists "$dep"; then
            log_message "INFO" "Optional dependency missing: $dep"
        fi
    done
...
# Download and extract ZIP with multiple methods
download_and_extract_zip() {
    ...
    if [ "$download_success" = false ] && command_exists wget; then
        ERROR_CONTEXT["method"]="wget_zip"
        log_message "DEBUG" "Downloading with wget..."
    ...
    if command_exists unzip; then
        ERROR_CONTEXT["method"]="unzip_extract"
        log_message "DEBUG" "Extracting with unzip..."
    ...
    if command_exists python3; then
        ERROR_CONTEXT["method"]="python_extract"
        log_message "DEBUG" "Extracting with Python..."
...
load_configuration() {
    ...
    if [ -f "$config_file" ]; then
        ...
        INSTALL_CONFIG["skip_validation"]="${INSTALL_CONFIG_SKIP_VALIDATION:-false}"
        INSTALL_CONFIG["cleanup_on_error"]="${INSTALL_CONFIG_CLEANUP_ON_ERROR:-true}"
...
INSTALL_CONFIG_SKIP_VALIDATION="${INSTALL_CONFIG[skip_validation]}"
INSTALL_CONFIG_CLEANUP_ON_ERROR="${INSTALL_CONFIG[cleanup_on_error]}"
EOF
...
# Get components for installation mode
