...
interface UseQpicMediaManagerOptions {
...
export function useQpicMediaManager(options: UseQpicMediaManagerOptions = {}) {
  const {
    initialItems = [],
    maxItems = 10,
    onItemsChange,
  ...
  const getFileInfoByCid = useCallback(
    async (cid: string) => {
      try {
...
export default useQpicMediaManager;
