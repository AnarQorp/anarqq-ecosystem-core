...
3. Check memory usage: `ps aux | grep qmail`
...
# Backup message data
mongodump --db messages --out backup/messages-$(date +%Y%m%d)
```
...
mongorestore backup/messages-latest/
...
```bash
# Scale up
docker-compose up -d --scale qmail=3

# Scale down
docker-compose up -d --scale qmail=1
```
...
- **Primary Contact**: qmail-team@q.network
