...
- `QDRIVE_MODE`: Deployment mode (standalone|integrated|hybrid)
- `QDRIVE_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `QDRIVE_JWT_SECRET`: JWT signing secret
- `QDRIVE_ENCRYPTION_KEY`: Local encryption key
- `QDRIVE_API_KEY`: API authentication key

### Performance
- `QDRIVE_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `QDRIVE_RATE_LIMIT_MAX`: Max requests per window
- `QDRIVE_TIMEOUT`: Request timeout (ms)
- `QDRIVE_MAX_CONNECTIONS`: Max concurrent connections
...
- `qdrive_requests_total`: Total HTTP requests
- `qdrive_request_duration_seconds`: Request duration histogram
- `qdrive_errors_total`: Total errors by type
- `qdrive_active_connections`: Current active connections
- `qdrive_queue_depth`: Current queue depth
