...
# Usage
client = QdriveClient('http://localhost:3008', 'your-squid-id')
...
// Add qdrive as a client
server.addClient('qdrive', {
  url: 'http://localhost:3008',
  tools: ['example']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qdrive operation
  const qdrive = new QdriveClient();
  const result = await qdrive.performOperation({
    squidId: identity.squidId,
...
// Listen for qdrive events
eventBus.subscribe('q.qdrive.*.v1', (event) => {
  console.log('Qdrive event:', event);
...
// Publish events to qdrive
eventBus.publish('q.qdrive.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
