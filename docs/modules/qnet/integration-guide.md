# QNET - Network Infrastructure API - Integration Guide
...
class qnetMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qnet',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qnet MCP server');
  }
...
// Usage
const mcpClient = new qnetMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qnetClient = axios.create({
  baseURL: 'http://localhost:3000/api/qnet',
  headers: {
...
// Example API call
const response = await qnetClient.get('/health');
...
eventBus.subscribe('q.qnet.*.v1', (event) => {
  console.log('Received event:', event);
  
  switch (event.type) {
    case 'q.qnet.created.v1':
      handleCreated(event.data);
      break;
    case 'q.qnet.updated.v1':
      handleUpdated(event.data);
      break;
    case 'q.qnet.deleted.v1':
...
import { qnetClient } from '@anarq/qnet';
...
const result = await qnetClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qnet-storage'
});

// Store encrypted data
await qnetClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qnetClient.someOperation();
});
```
...
describe('qnet Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new qnetClient({
      baseURL: 'http://localhost:3001/api/qnet',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await qnetClient.batch(operations);
```
