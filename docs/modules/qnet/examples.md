...
# Usage
client = QnetClient('http://localhost:3100', 'your-squid-id')
...
// Add qnet as a client
server.addClient('qnet', {
  url: 'http://localhost:3100',
  tools: ['qnet.ping']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qnet operation
  const qnet = new QnetClient();
  const result = await qnet.performOperation({
    squidId: identity.squidId,
...
// Listen for qnet events
eventBus.subscribe('q.qnet.*.v1', (event) => {
  console.log('Qnet event:', event);
...
// Publish events to qnet
eventBus.publish('q.qnet.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
