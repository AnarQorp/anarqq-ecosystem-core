...
### Core Configuration
- `QNET_PORT`: HTTP server port (default: 3100)
- `QNET_MODE`: Deployment mode (standalone|integrated|hybrid)
- `QNET_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `QNET_JWT_SECRET`: JWT signing secret
- `QNET_ENCRYPTION_KEY`: Local encryption key
- `QNET_API_KEY`: API authentication key

### Performance
- `QNET_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `QNET_RATE_LIMIT_MAX`: Max requests per window
- `QNET_TIMEOUT`: Request timeout (ms)
- `QNET_MAX_CONNECTIONS`: Max concurrent connections
...
- `qnet_requests_total`: Total HTTP requests
- `qnet_request_duration_seconds`: Request duration histogram
- `qnet_errors_total`: Total errors by type
- `qnet_active_connections`: Current active connections
- `qnet_queue_depth`: Current queue depth
