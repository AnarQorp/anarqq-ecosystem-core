...
class qmarketMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qmarket',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qmarket MCP server');
  }
...
// Usage
const mcpClient = new qmarketMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qmarketClient = axios.create({
  baseURL: 'http://localhost:3000/api/qmarket',
  headers: {
...
// Example API call
const response = await qmarketClient.get('/health');
...
import { qmarketClient } from '@anarq/qmarket';
...
const result = await qmarketClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qmarket-storage'
});

// Store encrypted data
await qmarketClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qmarketClient.someOperation();
});
```
...
describe('qmarket Integration', () => {
  let client;

  beforeEach(() => {
    client = new qmarketClient({
...
describe('qmarket Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new qmarketClient({
      baseURL: 'http://localhost:3001/api/qmarket',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await qmarketClient.batch(operations);
```
