...
### Core Configuration
- `QMAIL_PORT`: HTTP server port (default: 3090)
- `QMAIL_MODE`: Deployment mode (standalone|integrated|hybrid)
- `QMAIL_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `QMAIL_JWT_SECRET`: JWT signing secret
- `QMAIL_ENCRYPTION_KEY`: Local encryption key
- `QMAIL_API_KEY`: API authentication key

### Performance
- `QMAIL_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `QMAIL_RATE_LIMIT_MAX`: Max requests per window
- `QMAIL_TIMEOUT`: Request timeout (ms)
- `QMAIL_MAX_CONNECTIONS`: Max concurrent connections
...
```bash
curl http://localhost:3090/metrics
```

## Key Metrics

- `qmail_requests_total`: Total HTTP requests
- `qmail_request_duration_seconds`: Request duration histogram
- `qmail_errors_total`: Total errors by type
- `qmail_active_connections`: Current active connections
- `qmail_queue_depth`: Current queue depth
