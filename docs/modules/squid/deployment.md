...
### Core Configuration
- `SQUID_PORT`: HTTP server port (default: 3010)
- `SQUID_MODE`: Deployment mode (standalone|integrated|hybrid)
- `SQUID_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `SQUID_JWT_SECRET`: JWT signing secret
- `SQUID_ENCRYPTION_KEY`: Local encryption key
- `SQUID_API_KEY`: API authentication key

### Performance
- `SQUID_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `SQUID_RATE_LIMIT_MAX`: Max requests per window
- `SQUID_TIMEOUT`: Request timeout (ms)
- `SQUID_MAX_CONNECTIONS`: Max concurrent connections
...
- `squid_requests_total`: Total HTTP requests
- `squid_request_duration_seconds`: Request duration histogram
- `squid_errors_total`: Total errors by type
- `squid_active_connections`: Current active connections
- `squid_queue_depth`: Current queue depth
