# sQuid Identity API - Integration Guide
...
class squidMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/squid',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to squid MCP server');
  }
...
// Usage
const mcpClient = new squidMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const squidClient = axios.create({
  baseURL: 'http://localhost:3000/api/squid',
  headers: {
...
// Example API call
const response = await squidClient.get('/health');
...
const result = await squidClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'squid-storage'
});

// Store encrypted data
await squidClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await squidClient.someOperation();
});
```
...
describe('squid Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new squidClient({
      baseURL: 'http://localhost:3001/api/squid',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await squidClient.batch(operations);
```
