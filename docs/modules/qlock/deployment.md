...
# Test basic functionality
echo "Service running on port 3020"
```
...
### Core Configuration
- `QLOCK_PORT`: HTTP server port (default: 3020)
- `QLOCK_MODE`: Deployment mode (standalone|integrated|hybrid)
- `QLOCK_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `QLOCK_JWT_SECRET`: JWT signing secret
- `QLOCK_ENCRYPTION_KEY`: Local encryption key
- `QLOCK_API_KEY`: API authentication key

### Performance
- `QLOCK_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `QLOCK_RATE_LIMIT_MAX`: Max requests per window
- `QLOCK_TIMEOUT`: Request timeout (ms)
- `QLOCK_MAX_CONNECTIONS`: Max concurrent connections
...
```bash
curl http://localhost:3020/ready
```

## Liveness Check

```bash
curl http://localhost:3020/live
```
...
- `qlock_requests_total`: Total HTTP requests
- `qlock_request_duration_seconds`: Request duration histogram
- `qlock_errors_total`: Total errors by type
- `qlock_active_connections`: Current active connections
- `qlock_queue_depth`: Current queue depth
