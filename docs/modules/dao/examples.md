...
# Usage
client = DaoClient('http://localhost:3110', 'your-squid-id')
...
async function completeWorkflow() {
  ...
  // 3. Execute dao operation
  const dao = new DaoClient();
  const result = await dao.performOperation({
    squidId: identity.squidId,
...
// Publish events to dao
eventBus.publish('q.dao.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
