...
### Core Configuration
- `DAO_PORT`: HTTP server port (default: 3110)
- `DAO_MODE`: Deployment mode (standalone|integrated|hybrid)
- `DAO_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `DAO_JWT_SECRET`: JWT signing secret
- `DAO_ENCRYPTION_KEY`: Local encryption key
- `DAO_API_KEY`: API authentication key

### Performance
- `DAO_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `DAO_RATE_LIMIT_MAX`: Max requests per window
- `DAO_TIMEOUT`: Request timeout (ms)
- `DAO_MAX_CONNECTIONS`: Max concurrent connections
...
- `dao_requests_total`: Total HTTP requests
- `dao_request_duration_seconds`: Request duration histogram
- `dao_errors_total`: Total errors by type
- `dao_active_connections`: Current active connections
- `dao_queue_depth`: Current queue depth
