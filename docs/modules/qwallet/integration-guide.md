...
class qwalletMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qwallet',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qwallet MCP server');
  }
...
// Usage
const mcpClient = new qwalletMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qwalletClient = axios.create({
  baseURL: 'http://localhost:3000/api/qwallet',
  headers: {
...
// Example API call
const response = await qwalletClient.get('/health');
...
eventBus.subscribe('q.qwallet.*.v1', (event) => {
  console.log('Received event:', event);
  
  switch (event.type) {
    case 'q.qwallet.created.v1':
      handleCreated(event.data);
      break;
    case 'q.qwallet.updated.v1':
      handleUpdated(event.data);
      break;
    case 'q.qwallet.deleted.v1':
...
import { qwalletClient } from '@anarq/qwallet';
...
const result = await qwalletClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qwallet-storage'
});

// Store encrypted data
await qwalletClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qwalletClient.someOperation();
});
```
...
describe('qwallet Integration Tests', () => {
  let client;
...
const results = await qwalletClient.batch(operations);
```
