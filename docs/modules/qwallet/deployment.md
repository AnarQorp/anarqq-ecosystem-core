...
```bash
# Set integrated mode
export QWALLET_MODE=integrated
...
### Core Configuration
- `QWALLET_PORT`: HTTP server port (default: 3000)
- `QWALLET_MODE`: Deployment mode (standalone|integrated|hybrid)
- `QWALLET_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `QWALLET_JWT_SECRET`: JWT signing secret
- `QWALLET_ENCRYPTION_KEY`: Local encryption key
- `QWALLET_API_KEY`: API authentication key

### Performance
- `QWALLET_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `QWALLET_RATE_LIMIT_MAX`: Max requests per window
- `QWALLET_TIMEOUT`: Request timeout (ms)
- `QWALLET_MAX_CONNECTIONS`: Max concurrent connections
...
- `qwallet_requests_total`: Total HTTP requests
- `qwallet_request_duration_seconds`: Request duration histogram
- `qwallet_errors_total`: Total errors by type
- `qwallet_active_connections`: Current active connections
- `qwallet_queue_depth`: Current queue depth
