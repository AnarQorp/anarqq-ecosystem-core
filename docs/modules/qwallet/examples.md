...
# Usage
client = QwalletClient('http://localhost:3000', 'your-squid-id')
...
// Add qwallet as a client
server.addClient('qwallet', {
  url: 'http://localhost:3000',
  tools: ['wallet.sign']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qwallet operation
  const qwallet = new QwalletClient();
  const result = await qwallet.performOperation({
    squidId: identity.squidId,
...
// Listen for qwallet events
eventBus.subscribe('q.qwallet.*.v1', (event) => {
  console.log('Qwallet event:', event);
...
// Publish events to qwallet
eventBus.publish('q.qwallet.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
