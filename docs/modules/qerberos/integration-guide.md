# Qerberos Security & Audit API - Integration Guide
...
class qerberosMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qerberos',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qerberos MCP server');
  }
...
// Usage
const mcpClient = new qerberosMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qerberosClient = axios.create({
  baseURL: 'http://localhost:3000/api/qerberos',
  headers: {
...
// Example API call
const response = await qerberosClient.get('/health');
...
eventBus.subscribe('q.qerberos.*.v1', (event) => {
  console.log('Received event:', event);
  
  switch (event.type) {
    case 'q.qerberos.created.v1':
      handleCreated(event.data);
      break;
    case 'q.qerberos.updated.v1':
      handleUpdated(event.data);
      break;
    case 'q.qerberos.deleted.v1':
...
import { qerberosClient } from '@anarq/qerberos';
...
const hasPermission = await QonsentClient.check({
  ...
  resource: 'qerberos:resource:123',
  action: 'read'
});
...
const result = await qerberosClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qerberos-storage'
});

// Store encrypted data
await qerberosClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qerberosClient.someOperation();
});
```
...
describe('qerberos Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new qerberosClient({
      baseURL: 'http://localhost:3001/api/qerberos',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await qerberosClient.batch(operations);
```
