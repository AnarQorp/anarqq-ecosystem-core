...
```bash
# Scale up
docker-compose up --scale qerberos=3

# Kubernetes scaling
kubectl scale deployment qerberos --replicas=3

# Verify scaling
kubectl get pods -l app=qerberos
```
...
```bash
# Basic load test
ab -n 1000 -c 10 http://localhost:3050/health
