...
# Usage
client = QerberosClient('http://localhost:3050', 'your-squid-id')
...
// Add qerberos as a client
server.addClient('qerberos', {
  url: 'http://localhost:3050',
  tools: ['qerberos.audit']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qerberos operation
  const qerberos = new QerberosClient();
  const result = await qerberos.performOperation({
    squidId: identity.squidId,
...
// Listen for qerberos events
eventBus.subscribe('q.qerberos.*.v1', (event) => {
  console.log('Qerberos event:', event);
...
// Publish events to qerberos
eventBus.publish('q.qerberos.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
