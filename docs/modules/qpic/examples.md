...
# Usage
client = QpicClient('http://localhost:3070', 'your-squid-id')
...
// Add qpic as a client
server.addClient('qpic', {
  url: 'http://localhost:3070',
  tools: ['qpic.upload']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qpic operation
  const qpic = new QpicClient();
  const result = await qpic.performOperation({
    squidId: identity.squidId,
...
// Listen for qpic events
eventBus.subscribe('q.qpic.*.v1', (event) => {
  console.log('Qpic event:', event);
...
// Publish events to qpic
eventBus.publish('q.qpic.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
