...
### Core Configuration
- `QPIC_PORT`: HTTP server port (default: 3070)
- `QPIC_MODE`: Deployment mode (standalone|integrated|hybrid)
- `QPIC_LOG_LEVEL`: Logging level (debug|info|warn|error)
...
### Security
- `QPIC_JWT_SECRET`: JWT signing secret
- `QPIC_ENCRYPTION_KEY`: Local encryption key
- `QPIC_API_KEY`: API authentication key

### Performance
- `QPIC_RATE_LIMIT_WINDOW`: Rate limit window (ms)
- `QPIC_RATE_LIMIT_MAX`: Max requests per window
- `QPIC_TIMEOUT`: Request timeout (ms)
- `QPIC_MAX_CONNECTIONS`: Max concurrent connections
...
```bash
curl http://localhost:3070/ready
```

## Liveness Check

```bash
curl http://localhost:3070/live
```
...
- `qpic_requests_total`: Total HTTP requests
- `qpic_request_duration_seconds`: Request duration histogram
- `qpic_errors_total`: Total errors by type
- `qpic_active_connections`: Current active connections
- `qpic_queue_depth`: Current queue depth
