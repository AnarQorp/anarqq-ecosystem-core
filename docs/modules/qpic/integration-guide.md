...
class qpicMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qpic',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qpic MCP server');
  }
...
// Usage
const mcpClient = new qpicMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qpicClient = axios.create({
  baseURL: 'http://localhost:3000/api/qpic',
  headers: {
...
// Example API call
const response = await qpicClient.get('/health');
...
eventBus.subscribe('q.qpic.*.v1', (event) => {
  console.log('Received event:', event);
  
  switch (event.type) {
    case 'q.qpic.created.v1':
      handleCreated(event.data);
      break;
    case 'q.qpic.updated.v1':
      handleUpdated(event.data);
      break;
    case 'q.qpic.deleted.v1':
...
import { qpicClient } from '@anarq/qpic';
...
const result = await qpicClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qpic-storage'
});

// Store encrypted data
await qpicClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qpicClient.someOperation();
});
```
...
describe('qpic Integration', () => {
  let client;

  beforeEach(() => {
    client = new qpicClient({
...
describe('qpic Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new qpicClient({
      baseURL: 'http://localhost:3001/api/qpic',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await qpicClient.batch(operations);
```
