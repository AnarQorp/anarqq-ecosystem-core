...
class qchatMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qchat',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qchat MCP server');
  }
...
// Usage
const mcpClient = new qchatMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qchatClient = axios.create({
  baseURL: 'http://localhost:3000/api/qchat',
  headers: {
...
// Example API call
const response = await qchatClient.get('/health');
...
eventBus.subscribe('q.qchat.*.v1', (event) => {
  console.log('Received event:', event);
  
  switch (event.type) {
    case 'q.qchat.created.v1':
      handleCreated(event.data);
      break;
    case 'q.qchat.updated.v1':
      handleUpdated(event.data);
      break;
    case 'q.qchat.deleted.v1':
...
const result = await qchatClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qchat-storage'
});

// Store encrypted data
await qchatClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qchatClient.someOperation();
});
```
...
describe('qchat Integration', () => {
  let client;

  beforeEach(() => {
    client = new qchatClient({
...
describe('qchat Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new qchatClient({
      baseURL: 'http://localhost:3001/api/qchat',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await qchatClient.batch(operations);
```
