...
# Usage
client = QchatClient('http://localhost:3001', 'your-squid-id')
...
// Add qchat as a client
server.addClient('qchat', {
  url: 'http://localhost:3001',
  tools: ['qchat.post']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qchat operation
  const qchat = new QchatClient();
  const result = await qchat.performOperation({
    squidId: identity.squidId,
...
// Listen for qchat events
eventBus.subscribe('q.qchat.*.v1', (event) => {
  console.log('Qchat event:', event);
...
// Publish events to qchat
eventBus.publish('q.qchat.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
