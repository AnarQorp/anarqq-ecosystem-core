...
class qonsentMCPClient {
  constructor(options = {}) {
    this.client = new MCPClient({
      serverUrl: options.serverUrl || 'http://localhost:3000/mcp/qonsent',
      authentication: {
  ...
  async connect() {
    ...
    console.log('Connected to qonsent MCP server');
  }
...
// Usage
const mcpClient = new qonsentMCPClient({
  squidId: 'your-squid-id',
  token: 'your-jwt-token'
});
...
const qonsentClient = axios.create({
  baseURL: 'http://localhost:3000/api/qonsent',
  headers: {
...
// Example API call
const response = await qonsentClient.get('/health');
...
const result = await qonsentClient.getResource('123');
```
...
const encryptedData = await QlockClient.encrypt({
  data: sensitiveData,
  squidId: 'user-squid-id',
  purpose: 'qonsent-storage'
});

// Store encrypted data
await qonsentClient.store({
  data: encryptedData,
...
const result = await retryOperation(async () => {
  return await qonsentClient.someOperation();
});
```
...
describe('qonsent Integration Tests', () => {
  let client;
  ...
  beforeAll(async () => {
    ...
    client = new qonsentClient({
      baseURL: 'http://localhost:3001/api/qonsent',
      squidId: 'test-squid-id',
      token: 'test-token'
    });
  });
...
const results = await qonsentClient.batch(operations);
```
