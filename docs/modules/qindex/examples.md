...
# Usage
client = QindexClient('http://localhost:3040', 'your-squid-id')
...
// Add qindex as a client
server.addClient('qindex', {
  url: 'http://localhost:3040',
  tools: ['qindex.put']
});
...
async function completeWorkflow() {
  ...
  // 3. Execute qindex operation
  const qindex = new QindexClient();
  const result = await qindex.performOperation({
    squidId: identity.squidId,
...
// Listen for qindex events
eventBus.subscribe('q.qindex.*.v1', (event) => {
  console.log('Qindex event:', event);
...
// Publish events to qindex
eventBus.publish('q.qindex.command.v1', {
  squidId: 'user-id',
  action: 'process',
  data: { /* command data */ }
});
