...
```bash
# Scale up
docker-compose up --scale qmask=3

# Kubernetes scaling
kubectl scale deployment qmask --replicas=3

# Verify scaling
kubectl get pods -l app=qmask
```
...
```bash
# Basic load test
ab -n 1000 -c 10 http://localhost:3060/health
